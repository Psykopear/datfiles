...
notify_muted() {
    if $opt_use_dunstify; then
        ...
        volnoti-show -m
        # notify-send -i audio-volume-muted -t $expires -h int:value:0 -h string:x-canonical-private-synchronous:anything "Volume muted"
    fi
}
...
notify_volume() {
    ...
    if $opt_use_dunstify; then
        ...
        # notify-send -i "$icon" -t $expires -h int:value:"$vol" -h string:x-canonical-private-synchronous:anything "$text"
        volnoti-show $vol
    fi
}
